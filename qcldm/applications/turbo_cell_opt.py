...
class OptData:
	...
	def grad_step(self, vector_step):
		...
		xr = self.real_x(x)
		real_grad = (reduce(lambda s, i: s + i**2, xr, 0.))**0.5
	...
	def write_result(self, grad, real_grad, typ, damp):
		with open("coord.log", "a") as log:
			log.write("step = %5s | grad = %13.8f | full_grad = %13.8f | damp = %5.3f\n" % (typ, grad, real_grad, damp))
		
	def semistep(self, damp, vector_step):
		if self.first:
			self.dscf_step()
			grad, real_grad = self.grad_step(vector_step)
			self.write_result(grad, real_grad, 'vector' if vector_step else 'coord', damp)
			self.first = False
